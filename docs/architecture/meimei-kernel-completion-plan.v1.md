...
| **Dynamic loading** | `import()` via manifest — **Program:** [`../planning/kernel-app-separation-and-https-program.v1.md`](../planning/kernel-app-separation-and-https-program.v1.md); **ADR:** [`adr/ADR-001-app-runtime-v1.md`](adr/ADR-001-app-runtime-v1.md). **Delivered:** builtins (**`apps/*/meimei.app.json`**) + **`data/kernel/apps/registry.json`** dispatch (**default on**; **`MEIMEI_KERNEL_EXTERNAL_APPS=0`** disables); app auth (**MM-KERNEL-301**); all in-repo miniapps off static **`server.mjs`** imports (**MM-KERNEL-603** + **`meimei-dashboard-static-apps-import-check.mjs`**). |
